...
def merge(Pipeline p){
  ...
  def pr_merge_response = httpRequest authentication: 'cred-github', httpMode: 'PUT', url: "https://api.github.com/repos/${p.git_user}/${p.repository_name}/pulls/${p.pr_num}/merge", validResponseCodes: '200,405,409', wrapAsMultipart: false
  ...
  if (parsed_pr_merge_response.containsKey("merged")){
      if ("${parsed_pr_merge_response['merged']}" == "true") {
       ...
       def pr_check_response = httpRequest authentication: 'cred-github', httpMode: 'GET', url: "https://api.github.com/repos/${p.git_user}/${p.repository_name}/pulls/${p.pr_num}", validResponseCodes: '200,405,409', wrapAsMultipart: false
